    ...
    const fetchAI = async () => {
      ...
      try {
        const response = await fetch('/api/ai-travel/recommendations', {
          ...
          body: JSON.stringify({ from: 'Delhi', to: 'Mumbai', date: new Date().toISOString().slice(0,10), preferences: { transportType: 'bus' } })
        });
  ...
  return (
    <div className="pt-16 min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-8">Bus Booking</h1>
        <div className="bg-white rounded-lg shadow p-6">
          {loading ? (
            ...
            <div>
              <h2 className="text-xl font-semibold mb-2">AI Bus Recommendations</h2>
              {aiData.transportation && aiData.transportation.length > 0 ? (
                <div>
                  {aiData.transportation.filter(t => t.type === 'bus').map((t, i) => (
                ...
                <p className="text-gray-600">No bus recommendations available.</p>
              )}
            </div>
          ) : (
